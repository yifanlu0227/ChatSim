...
do
    python generate_sim_data.py --data_dir=/dssg/home/acct-umjpyb/umjpyb/ziwang/f2-nerf/data/waymo_multi_view/${file_name}
    cmd="python -u scripts/run.py --config-name=wanjinyou_big dataset_name=waymo_multi_view case_name=${file_name} exp_name=exp_1029 mode=render_path is_continue=true +work_dir=$(pwd)"
    echo "Running: $cmd"
    eval $cmd
    wait $!  # 等待上一个进程结束
done
