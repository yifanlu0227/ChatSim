...
//   Tensor trans = torch::exp(-acc_density);
//   Tensor weights = trans * alphas;
//   // std::cout << "sec_density" << sec_density.sizes() << std::endl;
//   Tensor last_trans = torch::exp(-FlexOps::Sum(sec_density, idx_start_end));
//   // std::cout << "sampled_colors" << sampled_colors.sizes() << std::endl;
//   // std::cout << "idx_start_end" << idx_start_end.sizes() << std::endl;
//   // std::cout << "idx_start_end" << idx_start_end.index({Slc(0, 10)}) << std::endl;
...
//   // std::cout << colors.sizes() << std::endl;
//   // std::cout << weights.sizes() << std::endl;
//   // std::cout << trans.sizes() << std::endl;
//   // std::cout << "last_trans" << last_trans.sizes() << std::endl;
//   // std::cout << sec_density.sizes() << std::endl;
...
RenderResult Renderer::Render(const Tensor& rays_o, const Tensor& rays_d, const Tensor& bounds, const Tensor& emb_idx) {
  ...
  if (bg_color_type_ == BGColorType::white) {
    ...
    if (global_data_pool_->mode_ == RunningMode::TRAIN) {
      ...
      // bg_color = torch::ones({n_rays, 3}, CUDAFloat) * .5f;
      bg_color = torch::tensor({0.0, 0.0, 0.0}, CUDAFloat).expand({n_rays, 3});
    }
  }
  else {
    bg_color = torch::zeros({n_rays, 3}, CUDAFloat);
  ...
  return { colors, sample_result_early_stop.first_oct_dis, disparity, edge_feat, depth, weights, idx_start_end, last_trans };
}
