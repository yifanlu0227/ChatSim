...
Dataset::Dataset(GlobalDataPool* global_data_pool) {
  ...
  // Load images
  {
    ...
    std::ifstream shutter_list(data_path + "/shutter_list.txt");
    for (int i = 0; i < n_images_; i++) {
      std::string image_path;
      std::getline(image_list, image_path);
      auto image = Utils::ReadImageTensor(image_path).to(torch::kCPU);

      float shutter;
      std::string shutter_path;
      std::getline(shutter_list, shutter_path);
      // std::cout << shutter_path << std::endl;
      std::ifstream file(shutter_path);
      std::string line;
      std::getline(file, line);
      file.close();
      // std::ifstream file(shutter_path);
      // shutter = std::stof(line) * 1000;
      // std::cout << config["shutter_coefficient"].as<float>() << std::endl;
      shutter = 1.0 + config["shutter_coefficient"].as<float>() * std::stof(line);
      Tensor shutter_tensor = torch::full({1280, 1920, 1}, shutter, CPUFloat).to(torch::kFloat32).to(torch::kCPU);
      auto result = torch::cat({image, shutter_tensor}, /*dim=*/2);
...
Rays Dataset::Img2WorldRay_wide_angle(int cam_idx, const Tensor &ij) {
  return Img2WorldRay_wide_angle(poses_[cam_idx], intri_[cam_idx], dist_params_[cam_idx], ij);
}

Rays Dataset::Img2WorldRay_wide_angle(const Tensor& pose,
                           const Tensor& intri,
                           const Tensor& dist_param,
                           const Tensor& ij) {
  int n_pts = ij.sizes()[0];
  ...
  float cx = intri.index({ 0, 2 }).item<float>() * 3;
  float cy = intri.index({ 1, 2 }).item<float>();
  float fx = intri.index({ 0, 0 }).item<float>();
  float fy = intri.index({ 1, 1 }).item<float>();
...
BoundedRays Dataset::RaysFromPose_wide_angle(const Tensor &pose, int reso_level) {
  int H = height_ / reso_level;
  int W = width_ / reso_level * 3;
  Tensor ii = torch::linspace(0.f, height_ - 1.f, H, CUDAFloat);
  Tensor jj = torch::linspace(0.f, width_ * 3 - 1.f, W, CUDAFloat);
  auto ij = torch::meshgrid({ ii, jj }, "ij");
  Tensor i = ij[0].reshape({-1});
  Tensor j = ij[1].reshape({-1});

  auto [ rays_o, rays_d ] = Img2WorldRay_wide_angle(pose, intri_[0], dist_params_[0], torch::stack({ i, j }, -1));
  // TODO....
  float near = bounds_.index({Slc(), 0}).min().item<float>();
  float far  = bounds_.index({Slc(), 1}).max().item<float>();
...
std::tuple<BoundedRays, Tensor, Tensor> Dataset::RandRaysData(int batch_size, int sets) {
  ...
  Tensor gt_colors = image_tensors_.view({-1, 4}).index({ (cam_indices * height_ * width_ + i * width_ + j).to(torch::kLong) }).to(torch::kCUDA).contiguous();
