...
public:
  ...
  void Execute();
  void Train(bool output_hdr, bool use_shutter);
  void TestImages(bool output_hdr, bool use_shutter);
  ...
  std::tuple<Tensor, Tensor, Tensor> RenderWholeImage_weights(Tensor rays_o, Tensor rays_d, Tensor bounds);
  void RenderPath(bool output_hdr, bool use_shutter, bool wide_angle);
  void RenderPanorama(bool use_shutter);
  void VisualizeImage(int idx, bool output_hdr, bool use_shutter);
