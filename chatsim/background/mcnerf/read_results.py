...
exp_path = '/dssg/home/acct-umjpyb/umjpyb/ziwang/f2-nerf/exp'
exp_name = 'exp_1109_0.15'
psnr_list = []
ssim_list = []
lpips_list = []
...
for file_name in file_list:
    ...
    json_file_path = os.path.join(exp_path, file_name, exp_name, 'info.json')

    with open(json_file_path, 'r') as file:
        data = json.load(file)

    psnr_mean = data['psnr']['mean']
    ssim_mean = data['ssim']['mean']
    lpips_mean = data['lpips']['mean']

    # print(f'PSNR Mean: {psnr_mean}')
    # print(f'SSIM Mean: {ssim_mean}')
    # print(f'LPIPS Mean: {lpips_mean}')
...
print('mean lpips:', np.mean(np.array(lpips_list)))