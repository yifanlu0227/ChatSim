        ...
        try:
            ...
            for split, split_dir in evaulation.items():
                ...
                for method in os.listdir(split_dir):
                    ...
                    # if under gt_dir/renders_dir there are subdirectories, we need to go deeper
                    else:
                        for subdir in subdirs:
                            ...
                            full_dict[scene_dir][method][subdir].update({"SSIM": torch.tensor(ssims).mean().item(),
                                                                "PSNR": torch.tensor(psnrs).mean().item(),
                                                                "LPIPS": torch.tensor(lpipss).mean().item()})
                            per_view_dict[scene_dir][method][subdir].update({"SSIM": {name: ssim for ssim, name in zip(torch.tensor(ssims).tolist(), image_names)},
                                                                    "PSNR": {name: psnr for psnr, name in zip(torch.tensor(psnrs).tolist(), image_names)},
                                                                    "LPIPS": {name: lp for lp, name in zip(torch.tensor(lpipss).tolist(), image_names)}})
